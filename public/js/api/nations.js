...
btn_all_api.addEventListener('click', function () {
    call_all_api("http://127.0.0.1:8000/api/public/countries", this);
});

btn_random_api.addEventListener('click', function () {
    call_random_api('http://127.0.0.1:8000/api/public/countries/random', this);
});

btn_spesifik_api.addEventListener('click', function () {
    call_spesifik_api(`http://127.0.0.1:8000/api/public/countries/${search_input.value}`, this);
});

btn_test_token.addEventListener('click', function(){
    test_token(`http://127.0.0.1:8000/api/private/countries/random`, this, token_input.value);
});